...
if (-not $TransportOnly) {
    ...
    if ($missing.Count -gt 0) {
        $runtimeReport = [ordered]@{
            lane = 'runtime-differential'
            status = 'skipped-local-prerequisite'
            missing = $missing
            unexplainedDifferences = $null
        }
        $runtimeReport | ConvertTo-Json -Depth 4 | Set-Content -LiteralPath (Join-Path $artifacts 'runtime-differential-report.json') -Encoding utf8
        if ($RequireRuntimeLane) {
            throw "The required runtime differential lane is missing: $($missing -join ', ')."
        }
        Write-Host "Runtime differential skipped locally: $($missing -join ', ')."
    }
    else {
        Invoke-DotNetTest `
        ...
        $runtimeReport = [ordered]@{
            ...
            unexplainedDifferences = 0
            comparedFields = @(
                'HTTP status, selected headers, and body',
                'durable queue-observed output',
                'user log level and message',
                'middleware marker sequence'
            )
            reportedExclusions = @(
                'host logs',
                'generated IDs',
