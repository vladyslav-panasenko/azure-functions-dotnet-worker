...
- [ ] Runtime smoke report confirms real Core Tools/Azurite coverage for built-in HTTP, ASP.NET HTTP, and queue trigger/output behavior without claiming cross-runtime parity.
...
1. No unexplained serialized-transport mismatch, and the independent real-host smoke lane passes.
